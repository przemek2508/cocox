...
void TIM3_Config_ADC(void)
{
    ...
    NVIC_InitStructure.NVIC_IRQChannel 						= TIM3_IRQn; 																																				//co przerwie
    NVIC_InitStructure.NVIC_IRQChannelPreemptionPriority 	= 2; 																															//piorytet (0 najwiekszy) -3 jest dla resetu; -1, -2 bledy systemowe
    ...
    TIM3_TimeBase.TIM_Period        = 40000;						//(uint16_t)PERIOD;		// Obliczona wartosc do ktorej zlicza licznik
