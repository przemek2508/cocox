...
void DAC1_Config(void)
{
	...
	DAC_InitStructure.DAC_OutputBuffer   = DAC_OutputBuffer_Enable;	// Bufor zalaczony
 	...
 	DMA_InitStructure.DMA_Memory0BaseAddr    = (uint32_t)&buff_wyj1;	// DMA pobiera dane z tego bufora
 	...
 	DMA_InitStructure.DMA_PeripheralDataSize = DMA_MemoryDataSize_Word;	// 32-bit dana z peryferium
 	DMA_InitStructure.DMA_MemoryDataSize     = DMA_MemoryDataSize_Word;		// 16-bit dana do transmisjii po stronie pamiêci
 	DMA_InitStructure.DMA_Mode               = DMA_Mode_Normal;								//DMA_Mode_Circular;	// brak pêtli
 	...
 	/* podwójny buffer */
 	DMA_DoubleBufferModeConfig(DMA1_Stream6,(uint32_t)&buff_wyj2,DMA_Memory_0);
